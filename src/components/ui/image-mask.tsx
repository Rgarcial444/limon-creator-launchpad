...
const ImageMask = React.forwardRef<HTMLDivElement, ImageMaskProps>(
  ({ images, className = '' }, ref) => {
    // Asegurar que tenemos exactamente 9 imágenes, rellenando con la primera si es necesario
    const displayImages = [...images];
    while (displayImages.length < 9) {
      displayImages.push(images[0] || 'https://images.unsplash.com/photo-1498019559366-a1cbd07b5160?q=80&w=2058&auto=format&fit=crop');
    }
    ...
    return (
      <div ref={ref} className={className}>
        ...
        <section className='grid grid-cols-3 gap-4 md:gap-8 bg-background border rounded-lg p-3 md:p-5'>
          {displayImages.slice(0, 9).map((image, index) => (
            <figure key={index} style={{ clipPath: `url(#clip-pattern${index === 0 ? '' : index})` }} className="overflow-hidden">
              <img
                src={image}
                alt={`Imagen ${index + 1}`}
